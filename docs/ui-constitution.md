...
Reference products for feel: Linear (density and discipline), Whoop and Oura web dashboards (calm data presentation), Arc browser (restrained but alive motion). Match their quality bar, not their branding.

The signature element is the **trace**: every recommendation and metric can expand its provenance on demand as a monospace log of rule ids and the data rows they read. It reinforces trust and gives the app its instrument character.
...
| `surface-2` | `#1C2030` | Hover states, nested surfaces, command palette |
...
- No glassmorphism, no noise textures, no decorative iconography, no rainbow charts. Elevation is border plus background step; drop shadows only on overlays (palette, modals, toasts) at low spread.
...
- Max content width 720px for single-column screens; dashboard grids may use full width to 1440px.
...
- **Recommendation card.** Action in Inter 14/500; reason in Inter 14/400 `text-mid`; estimated time and target metric as mono 12 chips. A trace affordance expands the monospace rule log inline. Accept, dismiss, snooze as a single row; accept styled primary.
...
- **Command palette.** `surface-2`, center-top, low-spread shadow, mono input, matched substrings in `text-hi` against `text-mid`.
...
4. **Palette open**: command palette scales from 0.97 with fade, 150ms.
...
- Recommendation cards never move on refresh in a way that displaces what is about to be tapped.
...
- Command palette on `Cmd/Ctrl+K`; quick-add on `Cmd/Ctrl+J`; screen-local shortcuts documented in Settings.
